...
Le premier graphe que l'on obtient comporte des liens doubles (`r nrow(relation_graph[which_multiple(graph_relation),])`) et meme triples (cf. tableau \@ref(tab:poidsrelation)), c'est à dire que l'on peut avoir plusieurs relations entre deux noeuds (implantations). C'est logique vu que le relations et les implantations peuvent évoluer au cours du temps. C'est quelque chose que nous allons devoir observer plus en detail. Dans un premier temps, une solution est de les regrouper pour en faire un "poids", plus ce poids est importants plus ces implantations possèdent des relations (cela suppose de mettre en équivalance les relations sur le principe d'une relation = une relation).  

...
Je n'ai gardé pas gardé les relations ascendantes pour les analyses suivantes car elles sont identiques. 

Il n'y a que trois cas d'ecarts importants entre `date_start_min` et `date_start_man` representé par la figure \@ref(fig:verif-datemin-et-max). Les données manquantes ne sont pas prises en compte. `date_stop_min` et `date_stop_max` sont identiques pour les relations.
...
```{r, date-modalite, fig.cap="(ref:date-modalite)"}

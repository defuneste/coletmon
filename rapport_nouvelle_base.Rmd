...
En prenant un interval de 100 ans cenrtré sur 50 on obtient `r length(interval)` classes et l'on peut faire un graphe particulier pour chacune d'elles. Attention par defaut en cas d'egalité en degré je prend juste le premier vertex (Id_vertex_prim). 

```{r tableauevolgraph}

tableauevolgraph <- data.frame(Interval_temp = interval * 50, Id_vertex_prim = names(sapply(sapply(g.relation.100, plus_gros_vertex), `[`, 1)))

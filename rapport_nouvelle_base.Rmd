...
id_vertex_prim : correspond aux vertexes (idimplantation) avec le plus haut degré dans la composantes connexes. En cas d'egalité, suivez mon regard les reseaux de deux vertexes, c'est le premier qui est pris. 
...
comps_synthese$diametre <- sapply(comps, diameter)
...
DT::datatable(comps_synthese)

```

Pour les points suivants je n'ai pas gardé les `hiérarchique asc. Ecole` ni les `hiérarchique ascendante` car elles doublonnaient avec leur vis à vis descendantes. On obtient `r comps$no` composantes connexes. Le tableau \@ref(tab:compconexe) recense les composantes connexes en fonction de leur taille.

(ref:compconexe-caption) Nombre de composantes connexes par leur taille 

```{r, compconexe, fig.align="center"}
#knitr::kable(table(comps$csize), col.names = c("Taille des composantes", "Nombres de composantes"), caption = "(ref:compconexe-caption)",format = "html", table.attr = "style='width:50%;'")
```
